...
description: >-
  ...
  `ncore` (prepare input data), `asset-harvester` (extract 3D objects
  from driving clips), `nurec-fixer` (clean up rendered frames), or
  `physical-ai-datasets` (find a dataset on Hugging Face). Read this
  skill first whenever a user mentions NuRec, NRE, neural
  reconstruction, USDZ rendering, 3DGUT, gaussian splatting from
  driving logs, sensor sim, or asks "which skill should I use for
  X?". Do NOT use to actually run any of those pipelines yourself —
  defer to the sibling skill it points to. Trigger keywords: nurec,
...
Decide *which* skill to read next for a NuRec / Neural Reconstruction
task. Five sibling skills cover the full pipeline:
...
**Use this skill when:** the user mentions NuRec / NRE / 3DGUT /
USDZ / "render this clip" / "convert this bag" / "extract objects"
but the right sub-skill isn't obvious yet. Always read this first.

**Do NOT use this skill when:**

- The right sub-skill is already obvious (open it directly).
- The task isn't a NuRec task at all (this skill won't help).
- You need actual implementation steps — defer to the sub-skill that
  this index points you at.
...
| Write a new converter for a sensor setup that isn't already supported (drone, RGB-D camera, ROS 2 bag, COLMAP, …) | `ncore` |
...
1. `ncore` — convert the recording into NCore V4.
...
1. `ncore` — make sure you still have the original NCore clip on
   disk; Asset Harvester needs it to crop the object views.
...
1. `physical-ai-datasets` — download `PhysicalAI-NuRec-PPISP`
   ...
   comparisons).
2. `ncore` — only needed if you want to re-build the NCore shards.
   The dataset already ships with both COLMAP and NCore V4 versions,
   so usually you can skip this.
...
exists) or, for skills without one (`ncore`, `physical-ai-datasets`,
this index), use `hf auth whoami` or a
...
This index is read-only — it doesn't itself need any tooling. The
*real* prerequisites are owned by each sibling skill:

| Sibling skill | Hard requirements |
|---------------|-------------------|
...
applicable) — run it before invoking the workflow. Never echo secret
env vars; see [Secrets handling across every sibling
skill](#secrets-handling-across-every-sibling-skill).

## Limitations

- **This skill only routes**; it does not run pipelines. If the user
  asks for hands-on steps, open the sibling skill that this index
  points at.
- **The catalogue is hand-curated.** A newly-added sibling skill
  will not be discoverable here until someone updates [Pick a
  skill](#pick-a-skill).
- **Names are portable, paths are not.** Cross-skill links use
  relative paths that work in the canonical repo
  (`.agents/skills/<name>/SKILL.md`). If the index is imported into
  a different runtime layout, prefer name-based skill resolution
  over file paths.
- This skill **does not enumerate Omniverse / Isaac Sim integration
  steps** — those live in upstream Omniverse / Isaac docs, not in
  the NuRec skill family.
...
| Sibling skill not found on disk | The host repo only has the index | Follow [Find a skill that isn't already on disk](#find-a-skill-that-isnt-already-on-disk). |
| Stale link to `ncore-data-conversion` | Older snapshots used that name; the skill is now `ncore` | Update the link to `ncore`. |
| User wants to delete disk artifacts | NuRec workflow caches grow large | Walk [Cross-skill teardown](#cross-skill-teardown) in the documented order. |
